...
public class OgrenciResource
{
    ...
    // http://localhost:8080/ogrenci/deleteByNumber/{number}
    @PostMapping(path = "/deleteByNumber/{number}") public ResponseEntity<Object> deleteByNumber(@PathVariable(name = "number") int number)
    {
        ogrenciRepository.deleteByNumber(number);
        return new ResponseEntity<>("başarılı", HttpStatus.OK);
    }
